        ...
        const fetchPhysioData = async () => {
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    console.log('Fetched physio data:', docSnap.data());
                } else {
  ...
  if(isLogined){
        ...
        try {
          const res = await addDoc(physioRef,{
            amount: Number(physioData?.fees * diffDays),
            receiverId:physioData.id,
    ...
    const handlePayment = useCallback(async () => {
        ...
        const options = {
          ...
          amount: Number(physioData?.fees * diffDays) * 100,
          currency: "INR",
          name: "ARQ",
    ...
    return (
        <div className='mb-12'>
            <Banner title={''} subHeading1={'Home'} subHeading2={'Physio care Details'} />
            <div className='lg:flex lg:flex-row justify-center sm:flex sm:flex-col w-full px-6 md:px-12 mt-12 gap-6 mx-0'>
                    ...
                    <div className='bg-white py-2 px-2'>
                        {physioData ? (
                            <div className='flex gap-2'>
                                <div>
                                    <Image src={physioData.images.url} width={100} height={100} alt={physioData.images.name} className=" w-[100%]  h-[100%]" style={{ objectFit: 'cover' }} />
                                </div>
                                <div className=''>
                                    <div className='flex gap-2 items-center'>
                                        <h2 className='text-lg text-black font-semibold'>{physioData.name}</h2> {/*added text-black class*/}
                                        <div className='text-green-600'><FaCheckCircle /></div>
                                    </div>
                                    <div>
                                        <h3 className='text-sm text-[#1A3578]'>Education: {physioData.education} /{physioData.experience} Years Experience </h3>
                                    </div>
                                    <div>
                                        <h3 className='text-xs text-[#B1B1B1]'>Working at {physioData.workingAt} Hospital</h3>
                                    </div>
                                    ...
                                    <div className='  w-full mt-2'>
                                        <button className='bg-[#CDF27E] w-full text-black px-6 py-2  rounded-md' >Fee:{physioData.fees}/- (per day)</button>
                ...
                <div className='lg:w-[55%] sm:w-[100%] bg-white px-6'>
                    ...
                    <div className=' mt-6 md:mt-8'>
                        ...
                        <div className='mb-1 ml-2'>Amount: {Number(physioData?.fees * diffDays) || physioData?.fees}</div>
